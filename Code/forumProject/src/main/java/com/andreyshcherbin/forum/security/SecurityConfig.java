...
import static com.andreyshcherbin.forum.security.AccessRules.ADMINS_ROLES;
import static com.andreyshcherbin.forum.security.AccessRules.FOR_ADMINS;
import static com.andreyshcherbin.forum.security.AccessRules.FOR_AUTHORIZED_USERS;
import static com.andreyshcherbin.forum.security.AccessRules.FOR_EVERYONE;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	private void configureAccessRules(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers(FOR_AUTHORIZED_USERS).authenticated().antMatchers(FOR_ADMINS)
				.hasAnyAuthority(ADMINS_ROLES).antMatchers(FOR_EVERYONE).permitAll();
	}
	...
	private void configureRememberMe(HttpSecurity http) throws Exception {
		http.rememberMe().tokenValiditySeconds(2419200).key("forum-key");
	}
